                    ...
                    <table class="w-full table-auto mb-6">
                        ...
                        @foreach($tasks as $task)
                            <tr>
                                ...
                                <td class="border px-4 py-2 capitalize text-center">{{$task->status }}
                                    @if($task->status == 'pending')
                                        <form action="{{route('status',$task)}}" method="POST" class="capitalize bg-red-500 p-2 w-full">@csrf @method('PUT')
                                            <button type="submit">Done</button>
                                        </form>
                                    @endif</td>

                                <td class="border px-4 py-2 flex">
                                    <a class="bg-emerald-600 text-white px-4 py-2 mx-2 text-xs rounded" href="{{route('task.edit',$task->id)}}">Edit</a>
                                    <a class="bg-sky-600 text-white px-4 py-2 mx-2 text-xs rounded" href="{{route('task.show',$task->id)}}">View</a>
